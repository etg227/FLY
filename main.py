...
class FlyApp:
    ...
    def flush_logs(self):
        ...
        if pending:
            try:
                # The live log is display-only. Temporarily enable the Text
                # widget only while FLY appends/trims its own output, then lock
                # it again so typing cannot alter the visible diagnostic log.
