...
不是所有人都需要的独立需求（小众站点等）放在 `optional\` 目录：**随仓库分发、随更新维护，但默认不加载**。在主界面点「可选配置库...」按需启用，启用后才会出现在分流列表里（带「可选」标签）。启用状态存在本机设置中。
...
| 可选库 | `optional\` | 库里按需启用后才进列表 |
| 私人自定义 | `private\custom_profiles.json` | 本机独有，永不提交 |

## 社区配置

想贡献配置：大众需求提到 `rules\`，小众需求提到 `optional\`，JSON 格式相同。规则尽量收窄，不要使用兜底规则（整浏览器类配置需说明理由）。库中站点由第三方运营，收录仅为分流便利，不代表本项目认可其内容；使用请遵守所在地法律与站点条款。
