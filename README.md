...
- **自动选线 + 出口稳定**：首次启动筛选日本节点（Japan / JPN / JP / 日本 / Tokyo / Osaka / 🇯🇵）并并行测速选最快；之后**优先沿用上次的节点**，只有它失效或延迟恶化（默认 >1000ms，可在 `app_settings.json` 调 `sticky_max_delay_ms`）才重新选线 —— 出口 IP 保持稳定，避免频繁变换触发平台风控。订阅信息项（剩余流量、到期）不会被误选
